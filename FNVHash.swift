...
func fnv1<S:Sequence>(bytes: S) -> UInt where S.Iterator.Element == UInt8 {
...
func fnv1a<S:Sequence>(bytes: S) -> UInt where S.Iterator.Element == UInt8 {
    var hash = Constants.OffsetBasis
    for byte in bytes {
        hash ^= UInt(byte)
...
func fnv1(string: String) -> UInt {
    return fnv1(bytes: string.utf8)
}

/// Calculates FNV-1a hash from a String using it's UTF8 representation.
func fnv1a(string: String) -> UInt {
    return fnv1a(bytes: string.utf8)
}
